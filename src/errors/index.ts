...
export function UnknownPermission(message: string): AppError {
  return new AppError({ status: 400, code: 'unknown_permission', message, retryable: false });
}

export function ScopeMismatch(message: string): AppError {
  return new AppError({ status: 400, code: 'scope_mismatch', message, retryable: false });
}

export function InvalidProperty(message: string): AppError {
